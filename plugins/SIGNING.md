...
A `.mybiplugin` runs in MyBI only if its signature verifies (or the user consents to an
...
PUBLISHER_SEED=<seed> mybi-plugin publisher-sign mychart.mybiplugin --token countersign-you.json
mybi-plugin verify mychart.mybiplugin       # uses the public key — no secrets
```
